// Use environment variable for API URL, fallback to same origin
...
function displayResults(results) {
  ...
  document.getElementById("statNotApplicable").textContent =
    vs.not_applicable || 0;
  document.getElementById("statAntisemiticTrope").textContent =
    vs.antisemitic_trope || 0;
}
  ...
  claims.forEach((claim) => {
    ...
    card.innerHTML = `
            ...
            <div class="claim-details">
                <div class="claim-score">Score: ${
                  claim.score !== null ? Math.round(claim.score) : "N/A"
                }</div>
            </div>
            ${
              claim.rationale
                ? `<div class="claim-rationale">${escapeHtml(
                    claim.rationale
                  )}</div>`
                : ""
            }
